    ...
    def _handle_start_print_job(self):
        if self.print_task_config.print_task_config['filament_soft'][self.extruder_index] == True:
            ...
            if self.print_task_config.print_task_config['filament_type'][self.extruder_index] == 'TPU':
                ...
                elif self.print_task_config.print_task_config['filament_sub_type'][self.extruder_index].startswith(
                        '95'):
        ...
        self.reactor.update_timer(self.check_entangle_timer,
                                  self.reactor.monotonic() + 0.2 + 0.02 * self.extruder_index)
    ...
    def _check_entangle_event(self, eventtime):
        ...
        if self.reactor.monotonic() >= self.last_log_time + 5.0:
            ...
            if toolhead and toolhead.get_extruder().extruder_index == self.extruder_index:
                logging.info(
                    f"[entangle] e[{self.extruder_index}], pos:{new_position}, whl:{new_wheel_counts}, whl2:{new_wheel_2_counts} "
        ...
        if self.print_task_config.print_task_config[
            'filament_entangle_sen'] == print_task_config.ENTANGLE_SENSITIVITY_MEDIUM:
            global_detect_sen = ENTANGLE_GLOBAL_SENSITIVITY_MEDIUM
        elif self.print_task_config.print_task_config[
            'filament_entangle_sen'] == print_task_config.ENTANGLE_SENSITIVITY_LOW:
        ...
        if delta_position >= (self.detection_length * self.config['detect_factor'] * global_detect_sen):
            dest_delta_count = int(
                delta_position / (self.detection_length * self.config['detect_factor'] * global_detect_sen))
            is_tangled = False
            ...
            if is_tangled:
                ...
                self.gcode.respond_info(
                    "[filament_entangle_detect] extruder[%d] filament has tangled!" % (self.extruder_index))
                logging.info(
                    f"[filament_entangle_detect] extruder[{self.extruder_index}], length: {self.detection_length}, factor: {self.config['detect_factor']}, "
